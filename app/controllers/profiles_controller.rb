class ProfilesController < ApplicationController
    ...
    def display_profile
        key = params[:id]
        @user = User.find(key)
        @reviews =  Review.joins("INNER JOIN gadgets ON gadgets.id = reviews.gadget_id").select("gadgets.gadget_name ,reviews.*").where(reviews: {user_id: key}).order(id: "DESC")    
        @good_sum = Review.joins("INNER JOIN empathies ON empathies.review_id = reviews.id").joins("INNER JOIN users ON users.id = reviews.user_id").select("users.user_name, count(empathies.review_id) num_emp").group("users.user_name").order("num_emp desc").where(reviews: {user_id: key})
    end   
